	...
	public Result<?> getUserPermissionByToken(HttpServletRequest request) {
		...
		try {
			...
			if(!PermissionDataUtil.hasIndexPage(metaList)){
				SysPermission indexMenu = sysPermissionService.list(new LambdaQueryWrapper<SysPermission>().eq(SysPermission::getName,"综合云控")).get(0);
			...
			if(roleIndex!=null){
				List<SysPermission> menus = metaList.stream().filter(sysPermission -> "综合云控".equals(sysPermission.getName())).collect(Collectors.toList());
